...
在 CSS 世界中，很好的掌握动画能够给页面带来很多炫酷的特效，动画有简单的，也有复杂的，但是万变不离其宗，只要掌握了其中原理和用法，应用的话自然水到渠成。

## 二、动画组成

首先，我们来简单介绍一下 CSS 动画。

最新版本的 CSS 动画由规范 -- [CSS Animations Level 1](https://link.juejin.cn/?target=https%3A%2F%2Fwww.w3.org%2FTR%2F2018%2FWD-css-animations-1-20181011%2F) 定义。
...
![动画示例](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e225aa952aaa439faa6c291acc70ed78\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

这就是一个最基本的 CSS 动画，本文将从 animation 的各个子属性入手，探究 CSS 动画的方方面面。

### 1. animation-name / animation-duration
...
![animation-name](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8930285afabe45ad89b5e56565e23530\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![animation-delay](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8ab1f76e162e4583b99d8026d0808502\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/260758ade5c649e8bbb893318cb52e22\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/20f1e77b073249c8a74866448a2589fe\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://user-images.githubusercontent.com/8554143/137946058-cd27fdc0-45a6-4710-a5bd-f81a9ee77bd4.gif)
...
1. [纯 CSS 实现华为充电动画](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FvYExwvm)：

![纯 CSS 实现华为充电动画](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/75ef362be0a24c259c56ffd3ce788a7f\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

1. [纯 CSS 实现火焰动画](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FjJJbmz)：

![纯 CSS 实现火焰动画](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/20971446fdd349b1bcee8185d1a33721\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

以**纯 CSS 实现华为充电动画**为例子，简单讲解一下。

仔细观察这一部分，上升的一个一个圆球，抛去这里的一些融合效果，只关注不断上升的圆球，看着像是没有什么规律可言：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/db64f395adec42ddbfb04dfc01c23ad8\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f62a1fb306e74263a0c495247909f4e4\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://user-images.githubusercontent.com/8554143/138098999-aa157e82-6058-4377-94d3-ad5324d3b136.gif)

[CodePen Demo -- 利用范围随机 animation-duration 和 animation-delay 实现随机动画效果](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FJjyRYyR)
...
![cubic-bezier-timing-function](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e8a88ee5ec99483fbc7e63de45e75c3b\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

关于它们之间的效果对比：

![cubic-bezier-timing-function 对比](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/aa0947cb563943489df21e412d50aad6\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
这里有个非常好用的网站 -- [cubic-bezier](https://link.juejin.cn/?target=https%3A%2F%2Fcubic-bezier.com%2F%23.25%2C.1%2C.25%2C1) 用于创建和调试生成不同的贝塞尔曲线参数。

#### 三次贝塞尔曲线缓动对动画的影响

关于缓动函数对动画的影响，这里有一个非常好的示例。这里我们使用了纯 CSS 实现了一个钟的效果，对于其中的动画的运动，如果是 `animation-timing-function: linear`，效果如下：

![b](https://user-images.githubusercontent.com/8554143/146766119-1098b124-7dfe-4f80-8e4d-1daa2d743e52.gif)

而如果我们我把缓动函数替换一下，变成 `animation-timing-function: cubic-bezier(1,-0.21,.85,1.29)`，它的曲线对应如下：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d192a0618e0345cbbfe1fabfafd281e9\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

整个钟的动画律动效果将变成这样，完全不一样的感觉：

![](https://user-images.githubusercontent.com/8554143/146766093-2bc11c92-5a9e-4633-8e2a-5abd199e7db2.gif)

[CodePen Demo - 缓动不同效果不同](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FJjyxLMY)

对于许多精益求精的动画，在设计中其实都考虑到了缓动函数。譬如 linear 这个缓动，实际应用于某些动画中会显得很不自然，因为由于空气阻力的存在，程序模拟的匀速直线运动在现实生活中是很难实现的。因此对于这样一个用户平时很少感知到的运动是很难建立信任感的。这样的匀速直线运动也是我们在进行动效设计时需要极力避免的。

#### 2. step-timing-function
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e279544039864f87ab33b835b8f02d71\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/77d21b8e0be841928ffe7af8b62ec38e\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2498b784ab384b2285f3037de01d67ba\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

完整的代码你可以戳这里 -- [CodePen Demo -- Sprite Animation with steps()](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FJjrBqJZ)
...
当然，在 `steps()` 中，还有 `steps(6, start)` 和 `steps(6, end)` 的差异，也就是其中关键字 `start` 和 `end` 的差异。对于上述的无限动画而言，其实基本是可以忽略不计的，它主要是控制动画第一帧的开始和持续时长，比较小的一个知识点但是想讲明白需要比较长的篇幅。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c2af99ab08f34d18aab1d658d2c3c4ab\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/862a9662ee3041eda616d67466959ac8\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ba84f70440db4022a92e243e6851847d\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

整个 loading 的圈圈看上去好像也在旋转，实际上只是 20 帧关键帧在切换，整体的效果感觉更适合 Loading 的效果。

因此，两种动画效果都是很有必要掌握的，在实际使用的时候灵活尝试，选择更适合的。

上述 DEMO 效果完整的代码：[CodePen Demo -- Scale Loading steps vs linear](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FoNGMROO)
...
![](https://user-images.githubusercontent.com/8554143/148635816-4858dde3-8ea4-4ccb-a846-d9639a3de395.gif)
...
![](https://user-images.githubusercontent.com/8554143/148635988-c3543b79-e414-4183-b8f1-e3b4765e652f.gif)

完整的 DEMO 你可以戳这里：[CodePen Demo -- CSS Beer!](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2Fmikegolus%2Fpen%2FjJzRwJ)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1d00652e15e04bf9bad4b125add9e5d7\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

一句话总结，元素在动画时间之外，样式只受到它的 CSS 规则限制，与 @keyframes 内的关键帧定义无关。

1. `animation-fill-mode: backwards` 表现如图：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/769cb9bc10c0410d9557ae96a4684892\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

一句话总结，元素在动画开始之前（包含未触发动画阶段及 `animation-delay` 期间）的样式为动画运行时的第一帧，而动画结束后的样式则恢复为 CSS 规则设定的样式。

1. `animation-fill-mode: forwards` 表现如图：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/94bd5ee543b14a8b997f197aa5e7fb02\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

一句话总结，元素在动画开始之前的样式为 CSS 规则设定的样式，而动画结束后的样式则表现为由执行期间遇到的最后一个关键帧计算值（也就是停在最后一帧）。

1. `animation-fill-mode: both` 表现如图：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9dc3346c715345aa92bd72d4f5eb9da2\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

一句话总结，综合了 `animation-fill-mode: backwards` 和 `animation-fill-mode: forwards` 的设定。动画开始前的样式为动画运行时的第一帧，动画结束后停在最后一帧。

### 6. animation-iteration-count/animation-direction
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/cc9e0922988e46739427cb44b592eca3\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c4dde34b0bda4c9c84c6f620359cb3d1\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
![](https://user-images.githubusercontent.com/8554143/148764642-e4fd3ce6-ff67-4581-bf7a-f74904ea3a91.gif)
...
[CSS Cascading and Inheritance Level 5(Current Work)](https://link.juejin.cn/?target=https%3A%2F%2Fwww.w3.org%2FTR%2Fcss-cascade-5%2F%23cascade-sort)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/057c39e7803845f49559a34ff81e335c\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

按照上述算法，大概是这样：

过渡动画过程中每一帧的样式 > 用户代理、用户、页面作者设置的 !important 样式 > 动画过程中每一帧的样式优先级 > 页面作者、用户、用户代理普通样式。

然而，经过多个浏览器的测试，实际上并不是这样。
...
对于不同浏览器，感兴趣的同学可以利用我这个 DEMO 自行尝试，[CodePen Demo - the priority of CSS Animation](https://link.juejin.cn/?target=https%3A%2F%2Fcodepen.io%2FChokcoco%2Fpen%2FPowaXjM)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2cb967d0bffc4467a5e132be6ed1800b\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)

简化一下也就是这个图：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9e494c3ccc894e51858c35097314f18c\~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
* [【Web动画】CSS3 3D 行星运转 && 浏览器渲染原理](https://link.juejin.cn/?target=https%3A%2F%2Fwww.cnblogs.com%2Fcoco1s%2Fp%2F5439619.html)
* [Accelerated Rendering in Chrome](https://link.juejin.cn/?target=https%3A%2F%2Fwww.html5rocks.com%2Fzh%2Ftutorials%2Fspeed%2Flayers%2F%23disqus\_thread)
...
transition ，亦即过渡，字面意思上来讲，就是元素从这个属性的某个值过渡到这个属性的另外一个值，这是一个状态的转变，需要一种条件来触发这种转变，比如我们平时用到的 :hover、:focus、:checked、媒体查询或者 JavaScript。
...
![image](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/6/3/163c42601ede4b6e\~tplv-t2oaga2asx-zoom-in-crop-mark:1304:0:0:0.awebp)

我们来分析这一整个过程，首先 transition 给元素设置的过渡属性是 transform，当鼠标移入元素时，元素的 transform 发生变化，那么这个时候就触发了 transition，产生了动画，当鼠标移出时，transform 又发生变化，这个时候还是会触发 transition，产生动画。
...
在官方的介绍上 animation 这个属性是 transition 属性的扩展，弥补了 transition 的很多不足，并且能制作很多复杂的动画效果。

* transition 适合简单的一次性动画，并且需要特定的触发；
* animation 适合比较复杂的动画，不需要特定的触发；
* 用 js 同样可以操控元素的样式实现动画，除了 setTimeout 和 setInterval 以外，还有比较新的 requestAnimationFrame；

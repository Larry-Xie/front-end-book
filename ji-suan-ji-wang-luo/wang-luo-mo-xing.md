...
![TCP/IP 模型和 OSI 模型对应关系](<../.gitbook/assets/image (1) (1) (1) (1).png>)

![网络模型](<../.gitbook/assets/image (2) (1) (1) (1).png>)
...
* 在应用层产生**报文**。
* 运输层在报文的基础上加上一些首部信息构成了运输层**报文段**，首部信息可能包括：允许接收端运输层向上向适当的应用程序交付报文的信息以及差错检测位信息。
* 网络层在此基础上增加了如源和目的端系统地址等网络层首部信息，生成了网络层**数据包**。
* 数据链路层给数据包添加发送端 MAC 地址和接收端 MAC 地址后被封装成**数据帧**。
* 在物理层数据帧被封装成**比特流**，之后通过传输介质传送出去。
